...
(function() {
  ...
  TC.control.LayerCatalogSilmeFolders.prototype.register = function(map) {
    ...
    window.silmeLayerCatalog = this; //Silme - use window to ensure global accessibility
    silmeLayerCatalog = this; // Keep for backward compatibility
          ...
          */
          ...
          const reDraw = function(element) {
            return new Promise(function(resolve, reject) {
              setTimeout(function() {
                // Force a reflow by reading offsetHeight and offsetWidth (read-only properties)
                // Accessing these properties triggers a browser reflow without assignment
      ...
      self.div.addEventListener('click', TC.EventTarget.listenerBySelector(searchListElementSelector, function(evt) {
        ...
        if (li.classList.contains(TC.Consts.classes.CHECKED)) {
          ...
          if (layer.isCompatible(self.map.crs)) {
            ...
            this.showProjectionChangeDialog(self, layer);
          }
        }
      }));
        ...
        for (var j = 0, jj = as.length; j < jj; j++) {
          ...
          if (n === name) {
            ...
            for (var i = 0; i < self.layers.length; i++) {
              const layerFound = cercaLayers(self.layers[i], uid);
              if (layerFound != null) {
  ...
  const addLayerToMap = function(layer, layerName) {
    ...
    if (newLayer.isCompatible(self.map.crs)) {
      ...
      this.showProjectionChangeDialog(self, newLayer);
    }
  };
  ...
  TC.control.LayerCatalogSilmeFolders.prototype.render = function(callback) {
    ...
    // Ensure silmeLayerCatalog is set (in case render is called before register completes)
    window.silmeLayerCatalog = self;
    silmeLayerCatalog = self;
      ...
      Promise.all(promises).then(function() { //SILME 20210419
        ...
        renderParent.call(self, function() {
          ...
          if (!self.div.querySelector('.tc-ctl-lcat-tree').querySelector('.tc-ctl-lcat-loading-node') && !!self.div.querySelector('.tc-ctl-lcat-tree').querySelector('.tc-ctl-lcat-node')) {
            ...
            // Call callback if provided - this is called after all layers are rendered
            // The callback from renderBranch is already called for each layer individually
            // This callback is from renderParent and doesn't need a layer parameter
            if (TC.Util.isFunction(callback)) {
              callback();
            }
          }
        ...
        const AfageixTotsElsParesQueNoShaginAfegit = function(capaActualPare, capaActual) {
          // Use self (the control instance) if available, otherwise try window.silmeLayerCatalog
          var silmeLayerCatalogRef = self || window.silmeLayerCatalog || silmeLayerCatalog;
          if (!silmeLayerCatalogRef) {
            console.error('silmeLayerCatalog is not defined');
            return;
          }
          var catalog = silmeLayerCatalogRef.options;
            ...
            if (nodeActual.id == capaActualPare) {
              ...
              if (nodePare != "" &&
                ...
                if (!cercaIdDinsArray(realTree.children, nodePare)) {
                  ...
                  const nodePareAAfegir = AfageixTotsElsParesQueNoShaginAfegit(nodeActual.parentNode);
  ...
  function layerCatalogCarregat() {
    console.log('layerCatalogCarregat');
  }
})();
